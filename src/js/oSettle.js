    ...
    if (!this.ymmessage) {
        ...
        this.ymmessage.subscribe(orderNum, function(data){
            if (data.topic == orderNum && data.msg == "PayFinish") {
                top.location.href = config.html.oComplete+"?"+search;
            }
        });
    }
    //支付宝、网银支付
...
YMoSettle.prototype.render = function () {
    //  尾款页面的支付系统、订单信息都与订金页面相似，故直接使用...同样需要线路id和主订单编号
